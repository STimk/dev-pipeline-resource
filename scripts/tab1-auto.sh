...
# 不设兜底标记 — 如果 Claude Code 没写 step_3_done，下游会等超时报错
# 好过写假标记或盲目重试导致循环
echo ""
echo "⚠️ 检查 step_3_done 标记..."
if ! grep -q "^✅ step_3_done" .workspace/claude_task.md 2>/dev/null; then
    echo "   step_3_done 未写入 — Claude Code 可能遇到 API 错误"
    echo "   查看上方输出确认原因，修复后重新运行"
fi
echo ""
